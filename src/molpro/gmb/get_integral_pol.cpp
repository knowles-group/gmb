...
container<2,double> get_integral_pol(std::string filename, 
  ...
  std::vector<spin> v_spin = {alpha, beta, photon}; // vector containing possible spins
  ...
  for (libtensor::orbit_list<2, double>::iterator it = ol.begin();
    ...
    if (!skip) {
      ...
      if (o1 == o2) {
        ...
        case (o):
          for (size_t i = 0; i < 1; i++) {
            ...
            // ptr[i+i*nmax] = omega*(i); // what Uliana seems to use      
          }
          break;
        case (v):
          for (size_t i = 0; i < nmax; i++) {
            ...
            // ptr[i+i*nmax] = omega*(1+i); // UM        
          }
          break;
        case (b):
          for (size_t i = 0; i < 1+nmax; i++) {
            if (shift) 
              ptr[i+i*nmax] = omega*(0.5+i)-omega*(0.5); // with shift        
            else
              ptr[i+i*nmax] = omega*(0.5+i);
      ...
      // else if (o1 == o & o2 == v){ // just to try sth elese
      //   for (size_t i = 0; i < nmax; i++)
      //     ptr[i] = omega;
      // }
    }
...
container<4,double> get_integral_pol(std::string filename, 
  ...
  auto h2_o1o4o2o3 = get_integral_pol(filename, o1, o4, o2, o3, true); 
