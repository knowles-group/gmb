#ifndef PROBLEM_EOM_CCSD_H_
#define PROBLEM_EOM_CCSD_H_
...
private:
  mutable supercontainer<> m_identity; ///> identity matrix
...
public:
  ...
  void init() {
    ...
    auto if_oo = eom_ccsd_if_oo(m_tampl.m2get(t1), tau, m_ham.m2get(f_oo), m_ham.m2get(f_ov),
                   m_ham.m4get(i_ooov), m_ham.m4get(i_oovv));    
    auto if_ov = eom_ccsd_if_ov(m_tampl.m2get(t1), m_ham.m2get(f_ov), m_ham.m4get(i_oovv)); 
    auto if_vv = eom_ccsd_if_vv(m_tampl.m2get(t1), tau, m_ham.m2get(f_ov), m_ham.m2get(f_vv), 
    ...
    auto iw_ooov = eom_ccsd_iw_ooov(m_tampl.m2get(t1), m_tampl.m4get(t2), tau, if_ov,
                    m_ham.m4get(i_oooo), m_ham.m4get(i_ooov), m_ham.m4get(i_oovv), m_ham.m4get(i_ovov), m_ham.m4get(i_ovvv));
    ...
    auto iw_ovvv = eom_ccsd_iw_ovvv(m_tampl.m2get(t1), m_tampl.m4get(t2), tau, m_ham.m2get(f_ov),  
                    m_ham.m4get(i_ooov), m_ham.m4get(i_oovv), m_ham.m4get(i_ovov), m_ham.m4get(i_ovvv), m_ham.m4get(i_vvvv));
  ...
  void action(const CVecRef<container_t> &parameters, const VecRef<container_t> &actions) const override {
    for (int k = 0; k < parameters.size(); k++) {
      ...
      auto r2_new = eom_ccsd_r2(ccp.m2get("r1"), ccp.m4get("r2"), m_tampl.m4get(t2), m_int.m2get("if_oo"), m_int.m2get("if_vv"),  
                    m_ham.m4get(i_oovv), m_int.m4get("iw_oooo"), m_int.m4get("iw2_ooov"), m_int.m4get("iw_ovov"), m_int.m4get("iw_ovvv"), m_int.m4get("iw2_ovvv"), m_int.m4get("iw_vvvv"));
      a.set("r1", r1_new);
      a.set("r2", r2_new);
    }
  }
