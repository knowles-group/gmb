...
container<4,double> eom_ccsd_l2 (
    ...
    l2_new(i|j|a|b) = 
                    l0*i_oovv(i|j|a|b)
                      + asymm(a, b, 
                          contract(e, l2(i|j|a|e), if_vv(b|e))
                        - contract(m, l1(m|a), iw_ooov(i|j|m|b))
                        - 0.5*contract(e, i_oovv(i|j|e|b), 
                            contract(m|n|f, t2(m|n|e|f), l2(m|n|a|f))))
                     + asymm(i, j, 
                           - contract(m, l2(i|m|a|b), if_oo(m|j))
                           + contract(e, l1(i|e), iw_ovvv(j|e|b|a))
                     ...
                     + 0.5 * contract(e|f, l2(i|j|e|f), iw_vvvv(a|b|e|f));
